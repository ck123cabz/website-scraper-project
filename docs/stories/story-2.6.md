...
- [ ] Scrape homepage only (not full site) to optimize ScrapingBee usage
- [ ] Extract company infrastructure signals: About page, Team page, Contact page presence
- [ ] Process only Layer 1 survivors (URLs that passed domain analysis)

### AC2: Company Infrastructure Detection
- [ ] Detect presence of: About page, Team page, Contact page (minimum 2 of 3 required)
- [ ] Store detection results in `layer2_signals` JSONB field
- [ ] Log detected pages: "Found: About, Team (2/3 required pages)"

### AC3: Active Blog Validation
- [ ] Detect blog section existence on homepage
- [ ] Check recent post publish dates (within configurable threshold, default 90 days)
- [ ] Minimum requirement: 1 post within threshold to pass
- [ ] Store blog freshness in `layer2_signals.blog_last_post_date`

### AC4: Tech Stack Detection
- [ ] Identify analytics tools (Google Analytics, Mixpanel, etc.)
- [ ] Identify marketing platforms (HubSpot, Marketo, ActiveCampaign, etc.)
- [ ] Score based on professional tool presence (2+ tools = PASS)
- [ ] Store detected tools in `layer2_signals.tech_stack` array

### AC5: Professional Design Indicators
- [ ] Modern CSS framework detection (Tailwind, Bootstrap, Material UI)
- [ ] Responsive design check (viewport meta tag, media queries)
- [ ] Professional imagery assessment (stock photos, custom graphics)
- [ ] Store design score in `layer2_signals.design_quality` (1-10)

### AC6: Configuration Integration
- [ ] Load Layer 2 rules from Story 3.0 settings (`classification_settings.layer2_rules`)
- [ ] Configurable thresholds: blog_freshness_days, required_pages_count, min_tech_stack_tools
- [ ] Configurable pass/fail criteria per signal type

### AC7: Performance Target
- [ ] 70% pass rate of Layer 1 survivors (target: eliminate 30%)
- [ ] Processing time: <5 seconds per URL (including ScrapingBee request)
- [ ] Parallel processing: 5 concurrent URLs (respects ScrapingBee rate limits)

### AC8: Results Tracking
- [ ] Store `layer2_signals` (JSONB) with all detected signals
- [ ] Mark elimination: `elimination_layer = 'layer2'` if rejected
- [ ] Update job counters: `jobs.layer2_eliminated_count`
- [ ] Log Layer 2 decision reasoning: "REJECT Layer 2 - Missing required pages (1/3 found)"

## Tasks / Subtasks

- [ ] Task 1: Design Layer 2 Filter Service Architecture (AC: 1)
...
- [ ] Task 2: Implement Homepage Scraping Logic (AC: 1)
...
- [ ] Task 3: Company Infrastructure Detection (AC: 2)
...
- [ ] Task 4: Active Blog Validation (AC: 3)
...
- [ ] Task 5: Tech Stack Detection (AC: 4)
...
- [ ] Task 6: Professional Design Indicators (AC: 5)
...
- [ ] Task 7: Configuration Integration (AC: 6)
...
- [ ] Task 8: Database Schema Updates (AC: 8)
...
- [ ] Task 9: Decision Logic and Result Storage (AC: 7, 8)
...
- [ ] Task 10: Performance Optimization (AC: 7)
...
- [ ] Task 12: Integration with Worker Pipeline (AC: 1, 7)
