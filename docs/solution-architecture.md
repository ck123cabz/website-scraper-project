...
6. **Results Module** (`src/results/`)
   - ResultsService: Save URL processing results to Supabase
   - Result entity: URL, classification, score, cost, status
...
```typescript
{
  ...
  currentStage: 'fetching' | 'filtering' | 'classifying' | null;
...
│   ├── ClassificationService (orchestrator)
│   ├── PreFilterService (regex rules)
...
**ProcessingService:**
- Orchestrate URL processing flow:
  1. Scrape URL (via ScrapingService)
  2. Apply pre-filter (via PreFilterService)
  3. If pass: Classify (via ClassificationService)
  4. Track cost (via CostTrackerService)
  5. Save result (via ResultsService)
  6. Log activity (via LogsService)
...
**PreFilterService:**
- Apply regex rules to URL and content
- Rules examples:
  - Reject if URL contains `/tag/`, `/category/`, `/author/`
  - Reject if content < 500 characters
  - Reject if no contact page found
- Return: PASS or REJECT with reason

**ClassificationService:**
- Primary: Call Gemini API with classification prompt
- On Gemini failure: Call GPT API as fallback
- Parse LLM response (structured JSON)
- Extract: classification (suitable/not_suitable), score (0-1), reasoning
- Track cost per call
...
| **FR008** | Intelligent Pre-Filtering | ✅ Complete | PreFilterService + regex rules + <100ms execution + "Pre-filter REJECT" logs | ✅ Ready |
...
| **NFR002** | Processing Performance (20 URLs/min) | ✅ Complete | BullMQ concurrency:5 + rate limiting:20 URLs/min + ScrapingBee integration + pre-filtering <100ms + database indexes | ✅ Ready |
| **NFR003** | Cost Efficiency (≤$150/month) | ✅ Complete | Pre-filtering (40-60% LLM reduction) + Gemini primary (33% cheaper) + GPT fallback + real-time cost tracking + CostTrackerService + total estimate: $99-134/month | ✅ Ready |
...
- ✅ Story 2.3 (Pre-filtering engine): PreFilterService + regex rules + <100ms execution
- ✅ Story 2.4 (Gemini/GPT integration): ClassificationService + GeminiService + GPTService + dual-LLM strategy
- ✅ Story 2.5 (Worker processing): ProcessingWorker + ScrapingService + Supabase writes + Realtime triggers
